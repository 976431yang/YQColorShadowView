...
//给mainShadow产生阴影的View，frame和mainIMGV想通，不用mainIMGV是因为动画需要（有时需要关闭阴影）
...
//初始backView的frame
@property (nonatomic, assign) CGRect shadowRect;

//初始IMGV的frame
@property (nonatomic, assign) CGRect IMGVRect;
...
- (void)drawTheImage {
    ...
    if (self.image != nil) {
        ...
        [self.filter setValue:[NSNumber numberWithFloat:self.shadowWidth]
                       forKey:kCIInputRadiusKey];
        
        CGFloat scale = [UIScreen mainScreen].scale;
        CGRect outSize = CGRectMake(-self.shadowWidth * 4,
                                    -self.shadowWidth * 4,
                                    self.cutIMG.size.width * scale +
                                        self.shadowWidth * 8,
                                    self.cutIMG.size.height * scale +
                                        self.shadowWidth * 8);
        ...
        if (self.closeShadow) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                CGFloat mainX = (self.bounds.size.width -
                                 self.bounds.size.width * 0.5) / 2;
                CGFloat mainY = (self.bounds.size.height -
                                 self.bounds.size.height * 0.5) / 2;
                self.mainShadow.frame = CGRectMake(mainX, mainY,
                ...
                self.shadowRect = CGRectMake(showX, showY,
                                             showWidth,
                                             showHeight);
            });
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
                self.backView.frame = CGRectMake(showX, showY,
